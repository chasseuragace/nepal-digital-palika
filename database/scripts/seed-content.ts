...
async function getCategoryId(name: string, entityType: string): Promise<string> {
  const { data, error } = await supabase
    ...
    .is('palika_id', null)  // Global categories have palika_id = null
    .single()
...
async function seedBlogPosts() {
  ...
  if (adminError || !adminUsers || adminUsers.length === 0) {
    console.log('⚠️  No admin users found. Run: npm run setup:auth-admin')
  ...
  console.log(`👤 Using admin author: ${adminUsers[0].full_name}`)
...
async function main() {
  ...
  try {
    ...
    await seedEvents()
    await seedBlogPosts() // Now works with admin users
