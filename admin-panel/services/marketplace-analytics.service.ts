  ...
  async getBusinessAnalytics(palikaId: number): Promise<ServiceResponse<BusinessAnalytics>> {
      try {
        ...
        const byVerificationStatus = {
          pending: statusData?.filter((b: { verification_status?: string }) => b.verification_status === 'pending').length || 0,
          verified: statusData?.filter((b: { verification_status?: string }) => b.verification_status === 'verified').length || 0,
          rejected: statusData?.filter((b: { verification_status?: string }) => b.verification_status === 'rejected').length || 0,
          suspended: statusData?.filter((b: { verification_status?: string }) => b.verification_status === 'suspended').length || 0,
        }
