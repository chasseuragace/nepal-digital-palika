...
export async function PUT(
  ...
  try {
    ...
    // Check if palika can access verification workflow
    const validationRes = await tierValidationService.validateProductVerification(
      parseInt(palikaId),
      params.id
    )
    ...
    if (!validationRes.data?.canVerify) {
      return NextResponse.json(
        { error: validationRes.data?.reason || 'Verification not available for this tier' },
        { status: 403 }
      )
    }
